...
define(
        ...
        , TaskAddView)
    {
        QXSettingView = Backbone.View.extend({
            ...
            events: {
                "click #add" : "add",
                "click #userTree":"getRoleTree",
                "click #roleTree":"getMenuTree",
            ...
            submitMenu: function(){
                ...
                for(var i in menuNodes){
                    ...
                    if(menuNodes[i].level == 2){
                        obj.sid = menuNodes[i].sid;
                        obj.attr2 = menuNodes[i].attr2;
                        obj.level = menuNodes[i].level;
                    }else{
                        obj.sid = menuNodes[i].sid;
                        obj.level = menuNodes[i].level;
                    }
                ...
                for(var i in rolenodes){
                    roleId = rolenodes[i].sid
                }
                var menuModel = new Backbone.Model;
                menuModel.fetchEx({menus:menus},{
                    url : 'api/menu/editMenuByRoleId/'+roleId,
                    dataType: 'json',
                    success: function (data) {
                        console.log(data);
                        if(data && data.get("status") == "S"){
                        ...
                        userModel.fetchEx({},{
                            ...
                            success: function (data) {
                                if(data && data.get("status") == "S"){
                                    ...
                                    if(entity.roleName){
                                        var roleTree = $.fn.zTree.getZTreeObj("roleTree");
                                        var node = roleTree.getNodeByParam("attr1", entity.roleName, null);
                ...
                userModel.fetchEx({},{
                    ...
                    success: function (data) {
                        if(data && data.get("status") == "S"){
                            ...
                            for(var i in entity){
                                var menuTree = $.fn.zTree.getZTreeObj("menuTree");
                                var node = menuTree.getNodeByParam("attr1", entity[i].attr1, null);
